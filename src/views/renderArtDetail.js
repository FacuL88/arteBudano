...
export function renderArtDetail(artId) {
    ...
    if (!artwork) {
        main.innerHTML = `
            <div class='container' style='padding: var(--space-xl) 0; text-align: center;'>
                ...
                <button class='btn' onclick="window.renderGallery()">Volver</button>
            </div>
        `;
        return;
    }
    ...
    main.innerHTML = `
        <div class='container__detail'>
            <button class='btn-back' onclick="window.renderGallery()">Volver</button>
                ...
                <div class='detail__info'>
                    ...
                    <div class='detail__specs'>
                        <div class='spec-item'>
                            ...
                            <span class='spec-value'>${artwork.medidas}</span>
                        </div>
                        <div class='spec-item'>
                            <span class='spec-label'>Técnica:</span>
                            <span class='spec-value'>${artwork.tecnica}</span>
                        </div>
                        <div class='spec-item'>
                            <span class='spec-label'>Año:</span>
                            <span class='spec-value'>${artwork.año}</span>
                        </div>
                        <div class='spec-item'>
                            <span class='spec-label'>Disponibilidad:</span>
                            <span class='spec-value ${artwork.stock > 0 ? 'available' : 'sold'}'>${artwork.stock > 0 ? `${artwork.stock} disponible` : 'Agotado'}</span>
                        </div>
                    </div>
            ...
            <div class='detail__related'>
                ...
                <div class='related__gallery'>
                    ${array.filter(item => item.id !== artwork.id).slice(0, 3).map(item => `
                        <div class='related-item' onclick='showArtDetail(${item.id})'>
